        ...
        for i in range(configobj().iterations):
            ...
            if ((i + 1) % 3 == 0):
                ...
                _, cost_gen_g, acc_gen_g = session.run((mod_f.train_op, mod_f.cost, mod_f.accuracy),
                                                       {mod_f.z: z, mod_f.target_bin: target_gen_bin,
            ...
            # update the discriminator
                ...
                gen_x = session.run((mod_g.outputs),
                                    {mod_g.z: z, mod_g.target: target_gen, mod_g.target_bin: target_gen_bin})
                ...
                _, cost, acc = session.run((mod_d.train_op, mod_d.cost, mod_d.accuracy),
                                           {mod_d.target_bin: y, mod_d.target: t, mod_d.image_input: x})
